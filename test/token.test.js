...
describe("PaydirtGold.sol", () => {
  ...
  describe("Setting up the environment", () => {
    it("Should deploy an ERC20", async () => {
      const Contract = await ethers.getContractFactory("PaydirtGold");

      // Deploy NFT Contract and assign minter as MIMNTER_ROLE.
      const initialSupply = ethers.utils.parseEther("360000000"); //200000000
      this.erc20 = await Contract.connect(this.admin)
        .deploy(
      ...
      expect(await this.erc20.MINTER_ROLE()).to.equal(
        "0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6"
      );
      expect(await this.erc20.PAUSER_ROLE()).to.equal(
  ...
  describe("Test minting", () => {
    ...
    it("Mint more tokens should fail => invalid access", async () => {
      ...
      await expect(
        this.erc20.connect(this.treasury).mint(this.treasury.address, supply)
  ...
  describe("Test pauser", () => {
    ...
    it("Should fail to pause the contract", async () => {
      await expect(this.erc20.connect(this.minter).pause()).to.be.revertedWith(
    ...
    it("Should fail to transfer tokens when paused", async () => {
      ...
      await expect(
        this.erc20.connect(this.treasury).transfer(this.alice.address, amount)
