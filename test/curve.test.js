...
describe('BatchedBancorMarketMaker.sol: testing the Curve!', () => {
  ...
  describe('Setting up the environment', () => {
    it('Should deploy TokenERC20', async () => {
      ...
      this.token = await TokenContract.connect(this.admin).deploy(
      ...
      expect(await this.token.symbol())
        .to.equal('TTest1');
      expect(await this.token.decimals())
        .to.equal(18);
      expect(await this.token.MINTER_ROLE())
        .to.equal('0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6');
      expect(await this.token.PAUSER_ROLE())
    ...
    it('Should set the Curve as a valid minter', async () => {
      ...
      await this.token.connect(this.admin).grantRole(MINTER_ROLE, this.curve.address);
      expect(await this.token.hasRole(MINTER_ROLE, this.curve.address)).to.equal(true);
    })
  ...
  describe('Initializing the Curve', () => {
    ...
    it('Should add initial collateral to the curve', async () => {
      await this.dai.connect(this.treasury).transfer(
        this.curve.address,
        ethers.utils.parseEther(initialCurveLiquidity.toString())
      );

      await this.curve.connect(this.admin).addCollateralToken(
        this.dai.address,
      ...
      expect(await this.dai.balanceOf(this.curve.address))
        .to.equal(ethers.utils.parseEther(initialCurveLiquidity.toString()));
      expect(await this.curve.getCollateralToken(this.dai.address))
        .to.deep.equal([
          true,
          ethers.BigNumber.from(0),
          ethers.BigNumber.from(0),
  ...
  describe('Alice buying at the Curve for 100k', () => {
    ...
    it('Should not allow Alice to buy with wrong collateral', async () => {
      await expect(
        this.curve.connect(this.alice).openBuyOrder(
          this.alice.address,
          this.token.address,
          1
        )
    ...
    it('Should not allow Alice to sell since she has not bought yet', async () => {
      ...
      await expect(
        this.curve.connect(this.alice).openSellOrder(
          this.alice.address,
          this.dai.address,
          aliceBalance
        )
    ...
    it('Should allow Alice to buy at the curve for 100K TDAIs', async () => {
      const amount = ethers.utils.parseEther('100000');
      await this.dai.connect(this.alice)
        .approve(this.curve.address, amount);
  ...
  describe('Bob buying at the Curve for 100k', () => {
    it('Should transfer 100K test DAIs to Bob', async () => {
	    const amount = ethers.utils.parseEther('100000');
      await this.dai.connect(this.treasury)
        .transfer(this.bob.address, amount);
      expect(
        await this.dai.balanceOf(this.bob.address)
      ).to.equal(amount);
    })

    it('Should allow Bob to buy at the curve for 100K TDAIs', async () => {
      const amount = ethers.utils.parseEther('100000');
      await this.dai.connect(this.bob)
        .approve(this.curve.address, amount);
  ...
  describe('Alice selling at the Curve', () => {
    ...
    it('Should allow Alice to sell at the curve all his Tokens', async () => {
      const aliceBalance = await this.token.balanceOf(this.alice.address);
      await this.token.connect(this.alice).approve(this.curve.address, aliceBalance);
  ...
  describe('Bob selling at the Curve', () => {
    it('Should allow Bob to sell at the curve all his Tokens', async () => {
      ...
      await this.token.connect(this.bob).approve(this.curve.address, bobBalance);
  ...
  describe('Alice and Bob buying in same batch', () => {
    ...
    it('Should allow Alice and Bob to buy in same batch', async () => {
      ...
      await this.dai.connect(this.alice)
        .approve(this.curve.address, amountA);
      await this.dai.connect(this.bob)
        .approve(this.curve.address, amountB);
      const txA = await this.curve.connect(this.alice)
      ...
      const txB = await this.curve.connect(this.bob)
    ...
    it('Should allow Alice to sell at the curve all his Tokens', async () => {
      const aliceBalance = await this.token.balanceOf(this.alice.address);
      await this.token.connect(this.alice)
  ...
  describe('Economic Simulation: it should properly increase and decrease price as expected', () => {
    ...
    it('Should transfer 100M test DAIs to Charlie', async () => {
      ...
      expect(
        await this.dai.balanceOf(this.charlie.address)
      ).to.equal(amount);
    })
    ...
    for(var i = 0; i < simulationRuns; i++) {
      it('Should allow Charlie to buy at the curve for the simulation amount TDAIs', async () => {
        await this.dai.connect(this.charlie)
    ...
    it('Should allow Charlie to sell at the curve for all his balance in same steps', async () => {
      ...
      await this.token.connect(this.charlie)
  ...
  describe('Should properly revert on wrong order claims', () => {
    it('Should allow Alice to open buy at the curve for 10K TDAIs', async () => {
      ...
      await expect(
        this.curve.connect(this.alice)
        .claimCancelledBuyOrder(this.alice.address, batchIdA, this.dai.address)
      ).to.be.revertedWith('MM_BATCH_NOT_CANCELLED');
      await this.curve.connect(this.alice)
        .claimBuyOrder(this.alice.address, batchIdA, this.dai.address);
      expect(
        await this.token.balanceOf(this.alice.address)
      ).to.be.above(0);
    })
    ...
    it('Should allow Alice to sell at the curve all his Tokens', async () => {
      ...
      await expect(
        this.curve.connect(this.alice)
          .claimCancelledSellOrder(this.alice.address, batchIdA, this.dai.address)
      ).to.be.revertedWith('MM_BATCH_NOT_CANCELLED');
      await this.curve.connect(this.alice)
        .claimSellOrder(this.alice.address, batchIdA, this.dai.address);
  ...
  describe('Should properly react to collateral removal', () => {
    ...
    it('Should fail if trying to claim wrong cancelled order ', async () => {
      await expect(
        this.curve.connect(this.alice)
          .claimCancelledSellOrder(this.alice.address, this.batchIdA, this.dai.address)
      ).to.be.revertedWith('MM_NOTHING_TO_CLAIM');
      await expect(
        this.curve.connect(this.bob)
          .claimCancelledBuyOrder(this.bob.address, this.batchIdB, this.dai.address)
      ).to.be.revertedWith('MM_NOTHING_TO_CLAIM');
    })
    ...
    it('Should allow users to claim cancelled orders', async () => {
      ...
      await this.curve.connect(this.alice)
        .claimCancelledBuyOrder(this.alice.address, this.batchIdA, this.dai.address);
      await this.curve.connect(this.bob)
        .claimCancelledSellOrder(this.bob.address, this.batchIdB, this.dai.address);
      ...
      expect(
        await this.token.balanceOf(this.bob.address)
      ).to.equal(this.amountB);
    })

    it('Should fail to readd collateral with wrong Reserve Ratio', async () => {
      await expect(
        this.curve.connect(this.admin)
          .addCollateralToken(
            this.dai.address,
            ethers.BigNumber.from(0),
            ethers.BigNumber.from(0),
    ...
    it('Should allow admin to readd collateral', async () => {
      await this.curve.connect(this.admin)
        .addCollateralToken(
          this.dai.address,
          ethers.BigNumber.from(0),
          ethers.BigNumber.from(0),
  ...
  describe('Should properly allow admin to manage the curve', () => {
    ...
    it('Should allow Alice to buy at a reduced price', async () => {
      ...
      expect(
        await this.curve
          .getCollateralPricePPM(this.dai.address)
    ...
    it('Should allow Admin to update beneficiary', async () => {
      ...
      expect(
        await this.curve.beneficiary()
      ).to.equal(this.treasury.address);
    })
    ...
    it('Should fail to remove non whitelisted dai token as collateral', async () => {
      await expect(
        this.curve.connect(this.admin)
          .removeCollateralToken(this.token.address)
      ).to.be.revertedWith('MM_COLLATERAL_NOT_WHITELISTED');
    })
